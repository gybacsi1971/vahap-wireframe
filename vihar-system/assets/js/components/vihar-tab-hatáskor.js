/**
 * VAHAP - F-0064 Hatáskör és illetékesség vizsgálat Tab (TELJES VERZIÓ)
 * UCE-1793 - Hatáskör vizsgálat
 * Használat: <vahap-tab-hatáskor :active="activeTab === 'hatáskor'" :ugy="ugy" @update="handleUpdate"></vahap-tab-hatáskor>
 *
 * Funkciók:
...
const VahapTabHatáskor = {
    name: 'vahap-tab-hatáskor',
    ...
    data() {
        return {
            ...
            formData: {
                ...
                altalanosMegjegyzes: '',
                dontes: null,         // 'hatáskor_van' | 'hatáskor_nincs' | 'illetekesseg_nincs'
                dontesDatum: null,
                ellenorzoNeve: ''
            },
    ...
    computed: {
        ...
        statuszBadge() {
            ...
            if (this.formData.dontes === 'hatáskor_van') {
                return { text: 'Hatáskör biztosított', class: 'bg-success' };
            }

            if (this.formData.dontes === 'hatáskor_nincs') {
                return { text: 'Hatáskör hiányzik', class: 'bg-danger' };
            }

            if (this.formData.dontes === 'illetekesseg_nincs') {
                return { text: 'Illetékesség hiányzik', class: 'bg-danger' };
            }
    ...
    methods: {
        ...
        loadKriteriumok() {
            console.log('[F-0064] Kritériumok betöltése...');

            // VIHARMockData.parameterezo.ellenorzesi_lista_hataskor betöltése
            if (VIHARMockData?.parameterezo?.ellenorzesi_lista_hataskor) {
                ...
                console.log(`[F-0064] Betöltve ${this.kriteriumok.length} kritérium`);
                ...
                console.warn('[F-0064] Nincs paraméterezett kritérium lista!');
                // Fallback: Alapértelmezett kritériumok
                this.kriteriumok = [
                    {
                        id: 1,
                        megnevezes: 'Hatáskör biztosított',
                        leiras: 'A hatóság hatáskörébe tartozik az ügy',
                        sorrend: 1,
                        kotelezo: true,
                        aktiv: true
                    },
                    {
                        id: 2,
                        megnevezes: 'Illetékesség rendben',
                        leiras: 'Területi illetékesség megfelelő',
                        sorrend: 2,
                        kotelezo: true,
                        aktiv: true
                    }
        ...
        loadData() {
            if (this.ugy && this.ugy.hatáskor_data) {
                console.log('[F-0064] Mentett adatok betöltése');
                this.formData = { ...this.ugy.hatáskor_data };
        ...
        evaluateDecision() {
            ...
            if (osszesCriteriaTeljesul) {
                this.formData.dontes = 'hatáskor_van';
                this.formData.dontesDatum = new Date().toISOString().split('T')[0];
            } else {
                // Ellenőrzés: melyik hiányzik
                const hatáskorKriterium = this.formData.checks[1];
                const illetekessegKriterium = this.formData.checks[2];

                if (!hatáskorKriterium) {
                    this.formData.dontes = 'hatáskor_nincs';
                } else if (!illetekessegKriterium) {
                    this.formData.dontes = 'illetekesseg_nincs';
                } else {
                    this.formData.dontes = null;
                }
        ...
        save() {
            ...
            console.log('[F-0064] Hatáskör vizsgálat mentése:', this.formData);

            // Emit update esemény
            this.$emit('update', {
                tab: 'hatáskor',
                data: this.formData,
                status: this.formData.dontes === 'hatáskor_van' ? 'completed' : 'rejected'
            });

            this.hasChanges = false;
            alert('Hatáskör vizsgálat sikeresen mentve!');

            // Következő lépés ajánlása
            if (this.formData.dontes === 'hatáskor_van') {
                this.$emit('next', 'formai');
            }
        },

        // Alaphelyzetbe állítás
        reset() {
            if (confirm('Biztosan alaphelyzetbe állítja a hatáskör vizsgálatot?')) {
                this.formData = {
                    checks: {},
                    megjegyzesek: {},
                    altalanosMegjegyzes: '',
    ...
    mounted() {
        console.log('[F-0064] Hatáskör vizsgálat tab betöltve');
        ...
        <div v-show="active" class="vahap-tab">
            ...
            <div class="tab-content">
                ...
                <div class="alert alert-info">
                    ...
                    <strong>Vizsgálja meg</strong>, hogy a kérelem elbírálása a hatóság hatáskörébe és
                    illetékességébe tartozik-e. Mind a {{ kriteriumok.filter(k => k.kotelezo).length }}
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control"
                              ...
                              rows="3"
                              placeholder="Hatáskör vizsgálathoz kapcsolódó további megjegyzések..."
                              @input="markChanged"></textarea>
                </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Vizsgálat dátuma</label>
                        <input type="date"
                               class="form-control"
                ...
                <div v-if="formData.dontes" class="alert"
                     :class="{
                         'alert-success': formData.dontes === 'hatáskor_van',
                         'alert-danger': formData.dontes !== 'hatáskor_van'
                     }">
                    <div class="d-flex align-items-center">
                        <i class="bi fs-3 me-3"
                           :class="{
                               'bi-check-circle': formData.dontes === 'hatáskor_van',
                               'bi-x-circle': formData.dontes !== 'hatáskor_van'
                           }"></i>
                        <div>
